...
import { formatDistanceToNow, formatDistance, differenceInSeconds } from "date-fns";
...
const GenesisCard: React.FC<GenesisCardProps> = ({ genesis, onClick }) => {
    ...
    return (
        <>
            <div className="bg-[#1A1E23] rounded-xl cursor-pointer border border-cyan-500/50 transition-all duration-300 w-full backdrop-blur-sm" onClick={onClick}>
                {/* Top Row */}
                <div className="flex items-center justify-between px-4 py-2">
                    <div className="flex items-center gap-2">
                        <div className="relative w-14 h-14 rounded-full overflow-hidden flex-shrink-0 border-2 border-cyan-500/30">
                            <Image src={genesis.virtual.image?.url || "/placeholder.png"} alt={genesis.virtual.name} fill className="object-cover" unoptimized />
                        </div>
                        <div className="flex flex-col gap-2">
                            <div className="flex items-end gap-2">
                                <h3 className="text-lg font-semibold leading-none text-white font-['Outrun']">{genesis.virtual.name}</h3>
                                <span className="text-xs text-gray-400 font-light">${genesis.virtual.symbol}</span>
                                <FaExternalLinkAlt className="w-3 h-3 mb-[1px] ml-2 text-gray-400 cursor-pointer hover:text-cyan-500" />
                            </div>
                            <div className="flex items-center gap-1">
                                <span className="px-2 py-1 rounded-md text-xs bg-gray-800/50 border border-cyan-400/50 text-gray-300">Entertainment</span>
                    ...
                    {status === "live" && (
                        <button onClick={handleSnipe} className="px-3 py-1 rounded-lg bg-cyan-500/10 text-cyan-400 border-2 border-cyan-400/80 font-semibold hover:bg-cyan-500/20 transition-all duration-300 flex items-center gap-2">
            ...
            <SnipeModal isOpen={isSnipeModalOpen} onClose={() => setIsSnipeModalOpen(false)} genesisId={genesis.genesisId} name={genesis.virtual.name} walletAddress={address || ""} />
        </>
    );
};
