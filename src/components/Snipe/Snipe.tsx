  ...
  return (
    <div className="w-full h-full overflow-y-auto lg:overflow-y-hidden lg:h-full flex-1 flex sm:flex-row flex-col lg:px-14 sm:px-7 px-4 py-3 gap-4 justify-center">
      ...
      <div className="md:hidden w-full flex flex-row border-t border-primary-100 text-sm font-semibold">
        <button className={`basis-1/2 shrink-0 rounded py-2 ${bottomTab === "list" ? "bg-white/20 border border-t-none border-white/5" : ""}`} onClick={() => setBottomTab("list")}>List</button>
        <button className={`basis-1/2 shrink-0 rounded py-2 ${bottomTab === "dex" ? "bg-white/20 border border-t-none border-white/5" : ""}`} onClick={() => setBottomTab("dex")}>Dex</button>
      </div>
    </div>
  );
};
