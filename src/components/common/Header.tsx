...
const Header = () => {
    ...
    // "Alerts", "DCA"
    const { activeTab, address, setActiveTab, networkData, setAddress } = useLoginContext();
    const network = networkCards.filter((network) => network?.id == networkData?.chainId!);
            ...
            <div className="max-w-7xl sm:max-w-full mx-auto px-4 sm:px-14 ">
                <div className="flex items-center justify-between h-[4.8rem]">
                    ...
                    <div className="flex flex-row text-2xl gap-2 justify-center items-center  ">
                        <Image src="/Login/webLogo.png" alt="Dexter" height={26} width={26} />
                        <h3 className="text-[1.6rem] leading-[2.1rem] text-white  font-bold font-grotesk">DEXTER</h3>
                    </div>
                    ...
                    <div className="hidden md:flex items-center space-x-4">
                        <div className="flex space-x-6 mr-4">
                            {tabs.map((item) => (
                                <button key={item} onClick={() => handleTabClick(item)} className={`text-base font-normal transition-colors duration-200 ${activeTab == item ? "text-primary-100" : "text-white hover:text-primary-100/80"}`}>
                                    {item}
                                </button>
                            ))}
                        </div>
                            ...
                            <Menu as="div" className="relative inline-block text-left w-fit">
                                <div>
                                    <MenuButton className="flex items-center justify-center p-2">
                                        <img src={true ? "/alert/bell-on.png" : "/alert/bell-off.png"} alt="notifications" className="size-6 object-contain" />
                                    </MenuButton>
                                </div>
                                <Transition
                                    as={Fragment}
                                    ...
                                    <MenuItems className="absolute left-[-40px] mt-2 w-80 origin-top-right rounded-md border-2 border-primary-100/30  backdrop-blur-sm  bg-black drop-shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        ...
                                        <div className="py-1">
                                            <div className="flex items-center justify-between px-4 py-4">
                                                Alerts
                                                <img src={"/alert/settings.png"} alt="notifications" className="size-5 object-contain" />
                                            </div>
                                            <MenuItem>
                                                <button onClick={copyAddressHandler} className="flex w-full items-center px-4 py-5 text-sm text-white border-t border-primary-100/30 bg-green-500/30">
                                                    <img src={"/alert/bell-on.png"} alt="notifications" className="mr-3 h-5 w-5 border border-green-100 rounded-full" />
                                                    <div className="flex flex-col items-start justify-center">
                                                        <p className="text-white text-sm">Virtuals portfolio up by <span className="text-green-500">+10%</span></p>
                                                        <p className="opacity-70 text-xs">5 mins ago • Portfolio</p>
                                                    </div>
                                                </button>
                                            </MenuItem>
                            ...
                            <Menu as="div" className="relative inline-block text-left w-fit">
                                <div>
                                    <MenuButton className="flex flex-row py-2 px-4 w-[150px] items-center justify-between space-x-3 border border-primary-100 text-black text-base text-center font-bold rounded-md hover:bg-primary-100/10 transition-colors duration-200">
                                        <Image alt="Network" src={network[0]?.image} width={20} height={20} />
                                        <div className="flex flex-row space-x-1">
                                            <p className="text-white font-semibold text-sm">
                                                {address?.slice(0, 4)}....{address?.slice(-4)}
                                            </p>
                                        </div>
                                        <TiArrowSortedDown size="1.2rem" className=" text-white transition-all duration-300 rotate-0 group-aria-expanded:rotate-180!" />
                                    </MenuButton>
                                </div>
                                <Transition
                                    as={Fragment}
                                ...
                                >
                                    <MenuItems className="absolute right-0 mt-2 w-56 origin-top-right rounded-md border border-primary-100/30  backdrop-blur-sm  bg-black/25 drop-shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                        <div className="py-1">
                                            <MenuItem>
                                                <button onClick={copyAddressHandler} className="flex w-full items-center px-4 py-2 text-sm text-white hover:bg-primary-100/10">
                                            ...
                                            <MenuItem>
                                                <button
                                                    onClick={() => address && window.open(`https://basescan.org/address/${address}`, "_blank")}
                                            ...
                                            <MenuItem>
                                                <button onClick={() => setAddress(null)} className="flex w-full items-center px-4 py-2 text-sm  border-t border-primary-100/30  text-white hover:bg-primary-100/10">
                            ...
                            >
                                <MenuItems className="absolute right-0 mt-2 w-56 origin-top-right rounded-md  backdrop-blur-sm border-[1px] border-primary-100/20 shadow-lg ring-1  ring-opacity-5 focus:outline-none">
                                    <div className="py-1">
                                        {tabs.map((item) => (
                                            <MenuItem key={item}>
                                                <button onClick={() => handleTabClick(item)} className={`flex w-full items-center px-4 py-2 text-sm ${activeTab === item ? "text-primary-100" : "text-white hover:bg-primary-100/10"}`}>
                                                    {item}
                                                </button>
                                            </MenuItem>
                                        ))}
                                        {address && (
                                            <>
                                                ...
                                                <MenuItem>
                                                    <button onClick={() => address && window.open(`https://basecscan.com/address/${address}`, "_blank")} className="flex w-full items-center px-4 py-2 text-sm text-white hover:bg-primary-100/10">
                                                        <HiOutlineExternalLink className="mr-3 h-5 w-5" />
                                                        View on Explorer
                                                    </button>
                                                </MenuItem>
                                                <MenuItem>
                                                    <button onClick={() => setAddress(null)} className="flex w-full items-center px-4 py-2 text-sm text-white hover:bg-primary-100/10">
                                                        <HiOutlineLogout className="mr-3 h-5 w-5" />
                                                        Disconnect
                                                    </button>
                                                </MenuItem>
