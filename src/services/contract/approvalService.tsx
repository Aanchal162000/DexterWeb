...
class ApprovalService {
    ...
    async checkAllowance(params: Omit<ApprovalParams, "amount">): Promise<string> {
        try {
    ...
    async approveToken(params: ApprovalParams): Promise<ethers.providers.TransactionReceipt> {
        try {
            const { tokenAddress, amount, provider, decimals = 18, spenderAddress = BuyContract } = params;
            ...
            const tx = await tokenContract.approve(spenderAddress, ethers.utils.parseUnits(String(bufferedAmount), decimals));
    ...
    async approveVirtualToken(amount: string, provider: ethers.providers.Web3Provider): Promise<ethers.providers.TransactionReceipt> {
